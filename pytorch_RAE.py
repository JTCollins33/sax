...
dir_path = "./data/"
fpath = "./LSTM_results/"
models_path = "./pytorch_model_progress/2-2_deeper_adaptive/"
N_CLUSTERS=3
num_epochs=100
save_freq=2
batch_size=15
...
class REncoder(nn.Module):
    def __init__(self, num_features, hidden_size, num_layers, num_classes, device):
        ...
        self.r2 = nn.GRU(input_size=hidden_size, hidden_size=hidden_size, num_layers=num_layers, batch_first=True)
        self.m = nn.AdaptiveMaxPool2d((1,num_classes))
    ...
    def forward(self, input, lens, hidden):
        # x = nn.utils.rnn.pack_padded_sequence(input, lens, batch_first=True, enforce_sorted=False)
        x, hidden = self.r1(input, None)
        x, hidden = self.r2(x, hidden)
        # x, _ = nn.utils.rnn.pad_packed_sequence(x, batch_first=True)
        # x = x[:,-1,:]
        # out = self.lin1(x)
        out = self.m(x).squeeze()
        return out, hidden
...
class RAE(nn.Module):
    ...
    def forward(self, input, dim, lens, hidden):
        ...
        #only keep last hidden layer
        decoder_hidden = encoder_hidden[:1].clone()
    ...
    def initHidden(self):
        ...
        return Variable(torch.zeros([1, batch_size, self.hidden_size], dtype=torch.float), requires_grad=True)


#perform kmeans clustering
def km(layer):
    print("Performing clustering....\n")
    layer.kmeans_AE(clusters=N_CLUSTERS)
...
#     model = RAE(1, 5, 1, 1, N_FEATURES, device).to(device)
    # model = torch.load(get_newest_model())
    model = torch.load(models_path+"model_80_epochs", map_location=torch.device('cpu'))
            ...
            for i in indices:
                ...
                while(not(done)):
                    ...
                    for th in ths_batch:
                        ...
                        flipped_batch_list.append(np.flip(normalized_arr, axis=0))
                        batch_list.append(normalized_arr)
                    batch = torch.tensor(batch_list)

                    batch = batch.reshape(batch.shape[0], batch.shape[1], 1).to(device)
                    flipped_batch=torch.tensor(flipped_batch_list)
                    flipped_batch = flipped_batch.reshape(flipped_batch.shape[0], flipped_batch.shape[1], 1).to(device)

                    #pass into model
#                     output, hidden = model(batch, batch.shape[1], torch.tensor(seq_lens), hidden)
                    output, hidden = model(flipped_batch.to(device), flipped_batch.shape[1], torch.tensor(seq_lens).to(device), hidden.to(device))
    ...
    if(testing):
        for i in range(len(layers)):
            ...
            while(not(done)):
                ...
                batch_features, hidden = model.encoder(flipped_batch, torch.tensor(seq_lens).to(device), hidden)
