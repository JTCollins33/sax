...
def get_ths(hdf5_file, figs = None):
    ...
    for n,d in file['/ThermalHistories'].items():
        ths.append(ThermalHistory(np.array(d), int(n), figs))
...
#         tsbm = d3d_file[SYNTH_TSBM].value
        points = np.array(d3d_file[SYNTH_POINTS])
        dist = np.array(d3d_file[SYNTH_DIST])
        vector = np.array(d3d_file[SYNTH_VECTOR])
        tsbm = np.array(d3d_file[SYNTH_TSBM])
    else:
...
#         tsbm = d3d_file[REAL_TSBM].value
        points = np.array(d3d_file[REAL_POINTS])
        dist = np.array(d3d_file[REAL_DIST])
        vector = np.array(d3d_file[REAL_VECTOR])
        tsbm = np.array(d3d_file[REAL_TSBM])
